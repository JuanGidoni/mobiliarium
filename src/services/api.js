const BASE_URL = process.env.API_BASE_URL || 'https://analisi.transparenciacatalunya.cat/resource/rhpv-yr4f.json';
...
export async function getByCategory(categories) {
    if (!categories || (Array.isArray(categories) && categories.length === 0)) {
        throw new Error('Invalid categories provided. Must be a non-empty string or array.');
    }
    try {
